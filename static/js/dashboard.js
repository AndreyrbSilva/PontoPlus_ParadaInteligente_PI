...
async function carregarDados() {
    try {
        ...
        atualizarDashboard(); // Chama a primeira vez para carregar tudo
...
function preencherFiltroLinhas() {
    ...
    // Limpa opções antigas, mantendo apenas a padrão "Todas as Linhas"
    ...
    dadosLinhas.forEach(l => {
        ...
        // CORREÇÃO AQUI:
        // O app.py envia o "numero_linha" dentro do campo "linha_id" na rota /api/onibus.
        // Para o filtro funcionar, o value do option tem que ser o numero da linha também.
...
function atualizarDashboard() {
    ...
    // Lógica de Filtro
    if (linhaSelecionada) {
        // Filtra onde o ID da linha (que na vdd é o número, vindo do Python) é igual ao selecionado
        onibusFiltrados = onibusFiltrados.filter(o => 
            String(o.linha_id) === String(linhaSelecionada)
        );
    }

    // Atualiza tudo com a lista filtrada
...
function atualizarKPIs(onibus, linhaSelecionada) {
    ...
    // KPI 3: Total de Linhas (Lógica de Esconder)
    const elKpiLinhas = document.getElementById("kpiLinhas");
    if(elKpiLinhas) {
        // Encontra o elemento pai (o card inteiro, div.card-kpi)
        const cardLinhas = elKpiLinhas.closest('.card-kpi');
        
        if (linhaSelecionada) {
            // Se estiver filtrado, esconde o card inteiro
            cardLinhas.style.display = 'none';
        } else {
            // Se não estiver filtrado, remove o 'none' (volta ao padrão do CSS) e mostra o total
...
function gerarGraficos(onibus, sensores) {
    // Destroi gráficos anteriores se existirem para recriar com os novos dados
    if (chartOnibus) chartOnibus.destroy();
    if (chartSensores) chartSensores.destroy();
    if (chartAtraso) chartAtraso.destroy();

    // Paleta moderna e visível em dark/light mode
    const colors = {
        online: "#4ADE80",       
        offline: "#94A3B8",      
        manutencao: "#FBBF24",   
    ...
    // ----------------------------------------------------
    // GRÁFICO 1: Status da Frota (Rosca)
    // ----------------------------------------------------
    ...
    chartSensores = new Chart(document.getElementById("graficoSensores"), {
        ...
        data: {
            ...
            datasets: [{
                label: "Quantidade de Sensores", // O texto que aparecerá na legenda
                data: [ativo, inativo],
        ...
        options: {
            ...
            plugins: { 
                // AQUI FOI ALTERADO: Adicionado a configuração da legenda igual ao Rosca
                legend: { 
                    display: true,
                    position: 'bottom',
                    labels: { usePointStyle: true, boxWidth: 8, padding: 20, color: "#455774ff"}
